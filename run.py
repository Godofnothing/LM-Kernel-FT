...
def main():
    ...
    if model_args.linearize:
        assert training_args.trainer == 'standard', "Linearization assumes standard trainer."
        ...
        param_list = []
        for param in params:
            param_list.append(param.detach().clone().requires_grad_(param.requires_grad))
        param_list = tuple(param_list)

        model = linearize(_func, param_list, start=model_args.linearization_type)
